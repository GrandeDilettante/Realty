...
public class UserController {
    ...
    public ResponseEntity<?> addRealtyUser(@PathVariable final Long id,
        ...
        try {
            userService.addRealtyUser(id, realtyId);
            return new ResponseEntity<>("Данный обьект успешно добавлен ", NO_CONTENT);
        } catch (Exception e) {
            return new ResponseEntity<>("Не удалось добавить данный обьект ", BAD_REQUEST);
        }
    ...
    @GetMapping("/{id}/realties")
    public ResponseEntity<?> getRealtyId(@PathVariable final Long id) {
        if (!userService.existsById(id))
            return new ResponseEntity<>("Данный обьект не найден ", NOT_FOUND);
        else
            return ResponseEntity.ok().body(userService.getRealtyIdById(id));
    }

    @DeleteMapping("/{id}/users")
    public ResponseEntity<?> deleteRealtyUser(@PathVariable final Long id,
                                              @RequestParam final Long userId) {
        if (userService.deleteRealtyUser(id, userId) > 0)
            return new ResponseEntity<>("Данный обьект успешно удален ", NO_CONTENT);
        else
            return new ResponseEntity<>("Не удалось удалить данный обьект", BAD_REQUEST);
    }

    @GetMapping("/{id}/realties/names")
    public ResponseEntity<?> getRealtyNamesById(@PathVariable final Long id) {
        if (!userService.existsById(id))
            return new ResponseEntity<>("Данный обьект не найден ", NOT_FOUND);
        else
            return ResponseEntity.ok().body(userService.getRealtyNamesById(id));
    }

    @GetMapping("/{id}/DTO")
    public UserDTO getAllUsersDTO(@PathVariable final Long id) {
        return userService.getAllUsersDTO(id);
    }
}
